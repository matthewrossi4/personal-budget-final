...
function Dashboard() {
    ...
    useEffect(() => {
        ...
        axios.get(`${process.env.SERVER_ADDR}/api/budget`, {headers: {Authorization: `Bearer ${tkn}`},})
        .then((res) => {
            updateData(res);
        })
        .catch((error) => {
    ...
    const addBudgetItem = (event) => {
        ...
        axios.post(`${process.env.SERVER_ADDR}/api/budget`, {getItemName, getItemValue, color, getItemExpense},{headers: {Authorization: `Bearer ${token}`}})
        .then((res) => {
            updateData(res);
            setItemName("");
            setItemValue("");
