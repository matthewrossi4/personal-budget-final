...
function Dashboard() {
    ...
    useEffect(() => {
        ...
        axios.get("http://134.209.220.164:4000/api/budget", {headers: {Authorization: `Bearer ${tkn}`},})
        .then((res) => {
            updateData(res);
        })
        .catch((error) => {
    ...
    const addBudgetItem = (event) => {
        ...
        axios.post('http://134.209.220.164:4000/api/budget', {getItemName, getItemValue, color, getItemExpense},{headers: {Authorization: `Bearer ${token}`}})
        .then((res) => {
            updateData(res);
            setItemName("");
            setItemValue("");
